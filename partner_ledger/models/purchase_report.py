...
class PurchaseReportComp(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', string='Supplier Wise')
        ...
        if self.style != 'monthly':
                    ...
                    if self.product_groups and not self.product_id and not self.product_categ:
                        ...
                        for each_coll in self.env['supplier.ledger.customer'].search(
                            ...
                            if each_coll.product_id.parent_id == self.product_groups:
                                ...
                                print(each_coll.credit)
                                print(each_coll.debit)
                                today_total_cheques.append(product_line)
            ...
            if self.report_type == 'grouped':
                ...
                if self.product_group:
                    today_total_cheques = []
                    for each_coll in self.env['supplier.ledger.customer'].search(
                            [('company_id', '=', self.env.user.company_id.id),('product_id','!=',False), ('date', '>=', self.from_date),('date', '<=', self.to_date)]):
            ...
            self.report_lines = False
            all = self.env['supplier.ledger.customer'].search([('company_id', '=', self.env.user.company_id.id),('product_id','!=',False),('date', '>=', self.from_date),('date', '<=', self.to_date)])
            if all:
                if self.report_type == 'partner':
                    self.report_lines = False
                    if self.partner_id:
